...
//class common.DomEvent
var DomEvent=(function(){
	function DomEvent(){}
	__class(DomEvent,'common.DomEvent');
	DomEvent.clipboard=function(str){
		if(!DomEvent.textarea){
			DomEvent.textarea=Browser.document.createElement("textarea");
			Browser.document.body.appendChild(DomEvent.textarea);
			DomEvent.textarea.setAttribute('readonly','');
			DomEvent.textarea.style="position: fixed;left: 100%;";
		}
		DomEvent.textarea.value=str;
		DomEvent.textarea.select();
		DomEvent.textarea.setSelectionRange(0,DomEvent.textarea.value.length);
		if(Browser.document.execCommand("Copy")){
			console.log("复制成功！");
			}else{
			console.log("不支持复制到粘贴板功能");
		}
	}

	DomEvent.deleteButton=function(_this){
		for (var i=0;i < DomEvent.buttonContainerArr.length;i++){
			if(DomEvent.buttonContainerArr[i].laya==_this){
				if(DomEvent.buttonContainerArr[i].state){
					DomEvent.buttonContainerArr[i].dom.remove();
					DomEvent.buttonContainerArr[i].dom="";
				}
				return;
			}
		}
	...
	DomEvent.domEvent=function(_this,fun,parameter){
		if(!DomEvent.buttonContainer){
			DomEvent.buttonContainer=Browser.createElement("div");
			DomEvent.buttonContainer.style.position="absolute";
			DomEvent.buttonContainer.style.zIndex=1E5;
			Browser.container.appendChild(DomEvent.buttonContainer);
		};
		var transform=Utils.getTransformRelativeToWindow(_this,0,0);
		...
		buttonElement.style.transform=DomEvent.buttonContainer.style.webkitTransform="scale("+transform.scaleX+","+transform.scaleY+") rotate("+(Laya.stage.canvasDegree)+"deg)";
		buttonElement.style.left=transform.x+'px';
		buttonElement.style.top=transform.y+'px';
		buttonElement.onclick=function (){
			fun(parameter);
		}
		DomEvent.buttonContainer.appendChild(buttonElement);
		for (var i=0;i < DomEvent.buttonContainerArr.length;i++){
			if(DomEvent.buttonContainerArr[i].laya==_this){
				DomEvent.buttonContainerArr[i].dom.remove();
				DomEvent.buttonContainerArr[i].dom=buttonElement;
				DomEvent.buttonContainer.appendChild(buttonElement);
			}
		};
		var obj={laya:_this};
		obj.dom=buttonElement;
		DomEvent.buttonContainerArr.push(obj);
	}

	DomEvent.textarea=null;
	DomEvent.buttonContainer=null;
	DomEvent.buttonContainerArr=[];
	return DomEvent;
})()
...
var LayaSample=(function(){
	function LayaSample(){
		...
		{url:"res/Screensaver.json",type:"atlas"}];
...
//class ui.ButtonListUI extends laya.ui.View
var ButtonListUI=(function(_super){
	function ButtonListUI(){
		this.buttonList=null;
		ButtonListUI.__super.call(this);
	}

	__class(ButtonListUI,'ui.ButtonListUI',_super);
	var __proto=ButtonListUI.prototype;
	__proto.createChildren=function(){
		laya.ui.Component.prototype.createChildren.call(this);
		this.createView(ButtonListUI.uiView);
	}

	__static(ButtonListUI,
	['uiView',function(){return this.uiView={"type":"View","props":{"width":1920,"height":1080},"child":[{"type":"Rect","props":{"y":0,"x":0,"width":1920,"lineWidth":1,"height":1080,"fillColor":"#4c82af"}},{"type":"Image","props":{"y":0,"x":0,"width":90,"skin":"comp/img_blank.png","sizeGrid":"4,4,4,4","height":1080}},{"type":"List","props":{"y":0,"x":2,"width":87,"var":"buttonList","repeatX":1,"height":1078},"child":[{"type":"Box","props":{"y":0,"x":0,"renderType":"render","name":"render"},"child":[{"type":"Button","props":{"y":0,"x":0,"width":87,"skin":"comp/btn_choose.png","name":"name","label":"投票记录","height":42}}]}]}]};}
	]);
	return ButtonListUI;
})(View)


//class ui.DomButtonUI extends laya.ui.View
var DomButtonUI=(function(_super){
	function DomButtonUI(){
		this.qwer=null;
		this.qwe=null;
		this.btn=null;
		this.qwe2=null;
		this.btnDel=null;
		this.img=null;
		DomButtonUI.__super.call(this);
	}

	__class(DomButtonUI,'ui.DomButtonUI',_super);
	var __proto=DomButtonUI.prototype;
	__proto.createChildren=function(){
		laya.ui.Component.prototype.createChildren.call(this);
		this.createView(DomButtonUI.uiView);
	}

	__static(DomButtonUI,
	['uiView',function(){return this.uiView={"type":"View","props":{"width":1830,"height":1080},"child":[{"type":"Image","props":{"y":221,"x":25,"var":"qwer","skin":"Screensaver/8.jpg"}},{"type":"Box","props":{"y":128,"x":36},"child":[{"type":"Label","props":{"var":"qwe","text":"fvgdfhbdgffd","skin":"Screensaver/8.jpg","label":"label"}}]},{"type":"Button","props":{"y":587,"x":54,"var":"btn","skin":"comp/btn_choose.png","label":"label"}},{"type":"Box","props":{"y":47,"x":36},"child":[{"type":"TextInput","props":{"width":216,"var":"qwe2","skin":"Screensaver/8.jpg","label":"label","height":71}}]},{"type":"Button","props":{"y":700,"x":49,"var":"btnDel","skin":"comp/btn_choose.png","label":"label"}},{"type":"Image","props":{"y":123,"x":380,"var":"img","skin":"comp/btn_choose.png"}}]};}
	]);
	return DomButtonUI;
})(View)


//class ui.ScreensaverUI extends laya.ui.View
var ScreensaverUI=(function(_super){
	function ScreensaverUI(){
		ScreensaverUI.__super.call(this);;
	}

	__class(ScreensaverUI,'ui.ScreensaverUI',_super);
	var __proto=ScreensaverUI.prototype;
	__proto.createChildren=function(){
		laya.ui.Component.prototype.createChildren.call(this);
		this.createView(ScreensaverUI.uiView);
	}

	__static(ScreensaverUI,
	['uiView',function(){return this.uiView={"type":"View","props":{"width":1830,"height":1080}};}
	]);
...
*liuhe
*/
//class module.ButtonListView extends ui.ButtonListUI
var ButtonListView=(function(_super){
	function ButtonListView(){
		...
		ButtonListView.__super.call(this);
		this.viewList=new Array(this.buttonData.length);
		this._layer=new Sprite();
		this._layer.pos(90,0);
		this.addChild(this._layer);
		this.onSelect(0);
		this.buttonList.array=[];
		for (var i=0;i < this.buttonData.length;i++){
			this.buttonList.array.push({
				name:{label:this.buttonData[i].name}
			});
		}
		this.buttonList.vScrollBarSkin=null;
		this.buttonList.selectEnable=true;
		this.buttonList.scrollBar.elasticBackTime=150;
		this.buttonList.scrollBar.elasticDistance=200;
		this.buttonList.selectHandler=Handler.create(this,this.onSelect,null,false);
	}

	__class(ButtonListView,'module.ButtonListView',_super);
	var __proto=ButtonListView.prototype;
	__proto.onSelect=function(index){
		if(this._curViewIndex==index)return;
		var view=this.viewList[index];
		if(!view){
			view=new this.buttonData[index].view();
...
//class module.DomButtonView extends ui.DomButtonUI
var DomButtonView=(function(_super){
	function DomButtonView(){
		DomButtonView.__super.call(this);
		var _$this=this;
		this.img.skin="https://gss2.bdstatic.com/-fo3dSag_xI4khGkpoWK1HF6hhy/baike/whfpf%3D180%2C140%2C50/sign=35c7deab3212b31bc7399e69e0250742/cc11728b4710b9128eb6b412c8fdfc03934522e2.jpg";
		var qwe=new Image();
		qwe.skin="https://gss2.bdstatic.com/-fo3dSag_xI4khGkpoWK1HF6hhy/baike/whfpf%3D180%2C140%2C50/sign=35c7deab3212b31bc7399e69e0250742/cc11728b4710b9128eb6b412c8fdfc03934522e2.jpg";
		this.addChild(qwe);
		qwe.pos(380,300);
		console.log(this.qwer.displayHeight);
		console.log(this.qwer.displayWidth);
		this.timerOnce(3000,this,function(){
			qwe.x=200;
		})
		DomEvent.domEvent(qwe,DomEvent.clipboard,"gfgdghd");
		this.btnDel.on("click",this,function(){
			DomEvent.deleteButton(qwe);
		})
		this.btn.on("click",this,function(){
			DomEvent.domEvent(qwe,DomEvent.clipboard,"fgdsgdfgdf");
			DomEvent.domEvent(_$this.qwe2,DomEvent.clipboard,"fgdfghdhd");
		})
	}

	__class(DomButtonView,'module.DomButtonView',_super);
	return DomButtonView;
})(DomButtonUI)
...
*@author heheda
*/
//class module.ScreensaverView extends ui.ScreensaverUI
var ScreensaverView=(function(_super){
	function ScreensaverView(){
		/***********图片数组************/
		this.imagesArr=[];
		/*********屏保坐标范围*****************/
		this.Range={minX:0,minY:0,maxX:Laya.stage.width,maxY:Laya.stage.height};
		/*********屏保小图移动范围*****************/
		this.Rangeyd=500;
		/*********屏保一个流程时间*****************/
		this.scrTimer=6000;
		/************放大缩小的数值****************/
		this.scaleNum=0;
		ScreensaverView.__super.call(this);
		this.images=new Sprite();
		this.scaleImg=new Image();
		this.images.width=this.Range.maxX;
		this.images.height=this.Range.maxY;
		this.addChild(this.images);
		for(var a=0;a<28;a++){
			var img=new Image();
			img.skin="Screensaver/"+a+".jpg";
			if(img.width/img.height>=this.Range.maxX/this.Range.maxY){
				img.height=img.height/(img.width/(this.Range.maxX/5));
				img.width=this.Range.maxX/5;
				}else{
				img.width=img.width/(img.height/(this.Range.maxY/5));
				img.height=this.Range.maxY/5;
			}
			img.x=Math.floor(Math.random()*(this.Range.maxX-img.width));
			img.y=Math.floor(Math.random()*(this.Range.maxY-img.height));
		...
		this.imagesFun();
		this.timerLoop(this.scrTimer,this,this.imagesFun);
	}

	__class(ScreensaverView,'module.ScreensaverView',_super);
	var __proto=ScreensaverView.prototype;
	__proto.imagesFun=function(){
		var _$this=this;
		for (var i=0;i < this.imagesArr.length;i++){
			var numX=Math.floor(Math.random()*this.Rangeyd)-this.Rangeyd/2+this.imagesArr[i].imgX;
			if(numX<0)numX=0;
			else if(numX>this.Range.maxX-this.imagesArr[i].width)numX=this.Range.maxX-this.imagesArr[i].width;
			var numY=Math.floor(Math.random()*this.Rangeyd)-this.Rangeyd/2+this.imagesArr[i].imgY;
			if(numY<0)numY=0;
			else if(numY>this.Range.maxY-this.imagesArr[i].height)numY=this.Range.maxY-this.imagesArr[i].height;
			Tween.to(this.imagesArr[i].obj,{x:numX,y:numY},this.scrTimer);
		}
		this.scaleImg.skin=this.imagesArr[this.scaleNum].obj.skin;
		this.scaleImg.width=this.imagesArr[this.scaleNum].width;
		this.scaleImg.height=this.imagesArr[this.scaleNum].height;
		this.scaleImg.pivot(this.scaleImg.width/2,this.scaleImg.height/2);
		this.scaleImg.scale(0,0);
		this.scaleImg.centerX=0;
		this.scaleImg.centerY=0;
		Tween.to(this.scaleImg,{scaleX:4,scaleY:4},1000);
		this.timerOnce(this.scrTimer-1000,this,function(){
			Tween.to(_$this.scaleImg,{scaleX:0,scaleY:0},1000);
		})
		this.images.addChild(this.scaleImg);
		this.scaleNum++;
		if(this.scaleNum>=this.imagesArr.length-1){
			this.scaleNum=0;
		}
	}
